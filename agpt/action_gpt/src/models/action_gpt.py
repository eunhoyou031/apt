...
class ActionGPT(nn.Module):
    def __init__(
        ...
        self.action_chunk_queries = nn.Embedding(sequence_length*action_chunk_size, hidden_size)
    ...
    def forward(self, 
        ...
        if self.freeze_vision:
            with torch.no_grad():
                obs_embeddings, patch_embeddings = self.model_vision(rgb)  # CLS token + patch tokens
        else:
            obs_embeddings, patch_embeddings = self.model_vision(rgb)
        ...
        action_chunk_queries = action_chunk_queries.view(1, self.sequence_length*self.action_chunk_size, self.hidden_size)
        action_chunk_queries = action_chunk_queries.expand(batch_size, -1, -1)  # (b, sequence_length*action_chunk_size, h)
        ...
        stacked_inputs = torch.cat([
            ...
            action_chunk_queries         # (b, sequence_length*action_chunk_size, h)
        ], dim=1)  # (b, n_lang_tokens + prev_action_buffer_size + sequence_length*action_chunk_size, h)
        ...
        action_query_end = action_query_start + self.sequence_length * self.action_chunk_size
        action_query_hidden = hidden_states[:, action_query_start:action_query_end]  # (b, sequence_length*action_chunk_size, h)
        ...
        if self.pred_discrete_arm_action:
            arm_action_preds = self.pred_arm_act(action_embedding)  # (b, sequence_length*action_chunk_size, (act_dim-1)*3)
            arm_action_preds = arm_action_preds.reshape(batch_size, self.sequence_length, self.action_chunk_size, self.act_dim-1, 3)
        else:
            arm_action_preds = self.pred_arm_act(action_embedding)  # (b, sequence_length*action_chunk_size, act_dim-1)
            arm_action_preds = arm_action_preds.reshape(batch_size, self.sequence_length, self.action_chunk_size, self.act_dim-1)
            
        gripper_action_preds = self.pred_gripper_act(action_embedding)  # (b, sequence_length*action_chunk_size, 1)
        gripper_action_preds = gripper_action_preds.reshape(batch_size, self.sequence_length, self.action_chunk_size, 1)
