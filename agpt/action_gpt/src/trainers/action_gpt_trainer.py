...
class ActionGPT_Trainer:
    def __init__(
        ...
        self.action_chunk_size = action_gpt_config.get('action_chunk_size', 5)
    ...
    def calculate_loss(self, batch, train):
        ...
        # 이전 액션 버퍼 생성
        batch_size = batch['rgb_initial'].shape[0]
        
        if 'previous_actions' in batch:
            prev_actions = batch['previous_actions']  # 데이터셋에서 가져온 실제 이전 액션 사용
        else:
            if self.is_main and train:
                print("Warning: No previous actions found in batch. Using random initialization.")
            prev_actions = torch.randn(batch_size, self.prev_action_buffer_size, self.act_dim, device=self.device) * 0.1  # 이전 액션이 제공되지 않은 경우 기존 방식으로 무작위 초기화
        ...
        if self.action_gpt_config.get('pred_discrete_arm_action', False):
