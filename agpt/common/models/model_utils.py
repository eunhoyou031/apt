...
def load_action_gpt_policy(args):
    ...
    variant = {
        ...
        'pred_discrete_arm_action': action_gpt_config.get('pred_discrete_arm_action', False) # predict discrete arm actions for berkeley_fanuc_manipulation
    }
    variant.update(model_vision_basic_config)

    eva = ActionGPT_PolicyWrapper(
        policy=action_gpt,
        variant=variant,
