...
class LMDBDataset_for_ActionGPT(Dataset):
    def __init__(
        ...
        sequence_length=1, #start_ratio, end_ratio, 
        act_dim=7, 
        chunk_size=5, 
    ...
    def __getitem__(self, idx):
        ...
        prev_action_buffer_size = 10  # 설정값으로 변경 가능
        act_dim = getattr(self, 'act_dim', 7)  # act_dim이 없으면 기본값 7 사용
        previous_actions = torch.zeros(prev_action_buffer_size, act_dim)
        max_retries = 5
        retry_count = 0
        ...
        while retry_count < max_retries:
            try:
                ...
                previous_actions = torch.zeros(prev_action_buffer_size, self.act_dim)
                
                # previous actions 
                for i in range(1, prev_action_buffer_size + 1):
                    prev_idx = idx - i
                    if prev_idx >= self.start_step and loads(self.txn.get(f'cur_episode_{prev_idx}'.encode())) == cur_episode:
                        # 같은 에피소드 내에서만
                        previous_actions[prev_action_buffer_size - i] = self.extract_action(prev_idx)
                    # 이외의 경우는 기본값 0으로 유지
                ...
                return {
                    ...
                    "previous_actions": previous_actions,  # 추가된 이전 액션 정보
                    "idx": orig_idx
                }
        ...
        return {
            "lang": "dummy instruction",
            "rgb_initial": self.dummy_rgb_initial.clone(),
            "rgb_future": self.dummy_rgb_future.clone(),
            "actions": self.dummy_actions.clone(),
            "mask": self.dummy_mask.clone(),
            "latent_mask": self.dummy_latent_mask.clone(),
            "previous_actions": torch.zeros(10, self.act_dim),
            "idx": idx
        }
...
class LMDBDataset_for_ActionGPT_CALVIN(LMDBDataset_for_ActionGPT):
    def __init__(self, act_dim=7, sequence_length=1, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.sequence_length = kwargs.get('sequence_length', sequence_length)
        self.act_dim = kwargs.get('act_dim', act_dim)
...
class JsonDataset_for_ActionGPT_Video(Dataset):
    def __init__(
...
class NpzDataset_for_ActionGPT_Video(Dataset):
    def __init__(
        self, split, skip_frame, 
        sequence_length,
