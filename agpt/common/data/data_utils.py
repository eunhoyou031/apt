...
def load_dataset(data_config, extra_data_config):
    ...
    train_dataset = dataset_cls(split='train', act_dim=extra_data_config.get('act_dim', 7), sequence_length=extra_data_config.get('sequence_length', 1), **data_config)
    eval_dataset = dataset_cls(split='val', act_dim=extra_data_config.get('act_dim', 7), sequence_length=extra_data_config.get('sequence_length', 1), **data_config)
    
    if 'act_pred' in extra_data_config:
        data_config['do_extract_action'] = extra_data_config.pop('act_pred')
